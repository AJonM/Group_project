...
for(i in 1:nrow(spp_per_island)){
  ...
  if(length(temp) != 0){
    PNAS_island_subset@data[which(PNAS_island_subset@data$id_spatial_island %in% rownames(spp_per_island)[i]),"SIE_mammal"] <- length(temp)
  }
  else{
    PNAS_island_subset@data[which(PNAS_island_subset@data$id_spatial_island %in% rownames(spp_per_island)[i]),"SIE_mammal"] <- 0
  }
...
# Proportion of SIE
PNAS_island_subset$pSIE <- PNAS_island_subset$SIE_mammal/PNAS_island_subset$Richness_mammal
...
PNAS_island_subset$SIE_nonVol <- PNAS_island_subset$SIE_mammal - PNAS_island_subset$SIE_bats
...
island_data = as.data.frame(read_excel("/Users/elisabarreto/Downloads/Appendix_2-dryad.xlsx"))
island_data = as.data.frame(read_excel("Appendix_2-dryad.xlsx")) ### NEW: load Appendix 2 from dryad (https://doi.org/10.5061/dryad.hmgqnk9j2)
...
# All islands
island_data[,predictor[c(1:9)]] = apply(island_data[,predictor[c(1:9)]], 2, as.numeric) ### NEW! Make sure that only the quantitative predictors are being transformed
print(corrplot::corrplot(cor(island_data[,predictor[c(1:9)]]),
...
island_data_endemism[,predictor[c(1:9)]] = apply(island_data_endemism[,predictor[c(1:9)]], 2, as.numeric)
print(corrplot::corrplot(cor(island_data_endemism[,predictor[c(1:9)]]),
                         method = "color", type = "upper", order = "alphabet", number.cex = .7, addCoef.col = "black", tl.col = "black", 
...
island_data[,predictor[c(1:9)]] = scale(island_data[,predictor[c(1:9)]])

# Species richness
# Negative binomial fitted with glmer take way longer than with glmmTMB (3 hours vs 10 minutes) - results are similar
# But glmmTMB does not allow to simulate residuals in DHARMa considering the different random effects, which glmer does. 
...
final_model_glmer_Richness_mammal = glmer.nb(Richness_mammal~Area*Current_isolation + Temperature_mean + Temperature_sd + Past_isolation + Climate_velocity +
                                               Precipitation_sd + Precipitation_mean + Elevation_sd + (1|bioregion) +
                                               (0+Area*Current_isolation|bioregion) + (0+Temperature_mean|bioregion) + (0+Temperature_sd|bioregion) +
                                               (0+Past_isolation|bioregion) + (0+Climate_velocity|bioregion) + (0+Precipitation_sd|bioregion) + (0+Precipitation_mean|bioregion) +
                                               (0+Elevation_sd|bioregion), data = island_data)
...
final_model_glmer_Richness_bats = glmer.nb(Richness_bat~Area*Current_isolation + Temperature_mean + Temperature_sd + Past_isolation + Climate_velocity +
                                             Precipitation_sd + Precipitation_mean + Elevation_sd + (1|bioregion) +
                                             (0+Area*Current_isolation|bioregion) + (0+Temperature_mean|bioregion) + (0+Temperature_sd|bioregion) +
                                             (0+Past_isolation|bioregion) + (0+Climate_velocity|bioregion) + (0+Precipitation_sd|bioregion) + (0+Precipitation_mean|bioregion) +
                                             (0+Elevation_sd|bioregion), data = island_data)
...
final_model_glmmTMB_Richness_nonVol = glmmTMB(Richness_nonVol~Area*Current_isolation + Temperature_mean + Temperature_sd + Past_isolation + Climate_velocity +
                                          ...
                                          (0+Past_isolation|bioregion) + (0+Climate_velocity|bioregion) + (0+Precipitation_sd|bioregion) + (0+Precipitation_mean|bioregion) +
                                          (0+Elevation_sd|bioregion), data = island_data, family = nbinom2)
final_model_glmer_Richness_nonVol = glmer.nb(Richness_nonVol~Area*Current_isolation + Temperature_mean + Temperature_sd + Past_isolation + Climate_velocity +
                                               Precipitation_sd + Precipitation_mean + Elevation_sd + (1|bioregion) +
                                               (0+Area*Current_isolation|bioregion) + (0+Temperature_mean|bioregion) + (0+Temperature_sd|bioregion) +
                                               (0+Past_isolation|bioregion) + (0+Climate_velocity|bioregion) + (0+Precipitation_sd|bioregion) + (0+Precipitation_mean|bioregion) +
                                               (0+Elevation_sd|bioregion), data = island_data)
...
island_data_endemism$SIE_mammal_with0 <- island_data_endemism$SIE_mammal-1 
island_data_endemism[,predictor[c(1:9)]] = scale(island_data_endemism[,predictor[c(1:9)]])

final_model_SIE_mammal = glm.nb(SIE_mammal_with0~Area+Current_isolation + Temperature_mean + Temperature_sd + Past_isolation + Climate_velocity +
                                 Precipitation_sd + Precipitation_mean + Elevation_sd, data = island_data_endemism)
final_model_SIE_mammal = glm.nb(SIE_mammal_with0~Area+Current_isolation + Temperature_mean + Temperature_sd + Past_isolation + Climate_velocity +
                                 Precipitation_sd + Precipitation_mean + Elevation_sd, data = island_data_endemism, init.theta = final_model_SIE_mammal$theta)
...
final_model_SIE_nonVol = glm.nb(SIE_nonVol~Area+Current_isolation + Temperature_mean + Temperature_sd + Past_isolation + Climate_velocity +
                                  Precipitation_sd + Precipitation_mean + Elevation_sd, data = island_data_endemism)
final_model_SIE_nonVol = glm.nb(SIE_mammal_with0~Area+Current_isolation + Temperature_mean + Temperature_sd + Past_isolation + Climate_velocity +
                                 Precipitation_sd + Precipitation_mean + Elevation_sd, 
                               data = island_data_endemism, init.theta = final_model_SIE_nonVol$theta)

plot_models(final_model_SIE_mammal, final_model_SIE_bats,final_model_SIE_nonVol)


# pSIE
final_model_pSIE_mammal = glmer(pSIE_mammal~Area+Current_isolation + Temperature_mean + Temperature_sd + Past_isolation + Climate_velocity +
...
ss <- getME(final_model_pSIE_mammal,c("theta","fixef"))
final_model_pSIE_mammal <- update(final_model_pSIE_mammal,start=ss)
summary(final_model_pSIE_mammal)
...
final_model = list(final_model_glmmTMB_Richness_mammal, final_model_glmmTMB_Richness_bat, final_model_glmmTMB_Richness_nonVol,
                    final_model_SIE_mammal, final_model_SIE_bats, final_model_SIE_nonVol,
                    final_model_pSIE_mammal, final_model_pSIE_bats, final_model_pSIE_nonVol)
...
for(i in 1:length(final_model)){
  ...
  vars = colnames(model.frame(final_model[[i]])) ### NEW: the old line doesn't work anymore because glmmTBM has a different structure in the latest versions
  vars = vars[c(2:(length(vars)-1))]
  par(mfrow = c(3, ceiling(length(vars)/3)))
  for(j in 1:length(vars)){ # NEW: fixed the indexing to j
    plotResiduals(island_data[,vars[j]], simulated$scaledResiduals, xlab = vars[j])
  }
  # Plot residuals against predictor using true residuals
  par(mfrow = c(3, ceiling(length(vars)/3)))
  for(j in 1:length(vars)){ # NEW: fixed the indexing to j
    plot(island_data[,vars[j]], residuals(final_model[[i]], type = "pearson"), xlab = vars[j])
  }
  dev.off()
  # Check for outliers
...
#library(MuMIn)
for(i in 1:length(final_model)){
  print(MuMIn::r.squaredGLMM(final_model[[i]]))
}
...
SR = plot_models(final_model_glmmTMB_Richness_mammal,
                ...
                final_model_glmmTMB_Richness_nonVol, ### NEW: replaced glmer by glmmTMB models
            transform = NULL,
...
SIE = plot_models(final_model_SIE_mammal,
...
pSIE = plot_models(final_model_pSIE_mammal,
...
# Latitude
diff_latitude = lm(Latitude_centroid ~occupancy, data = island_data) # NEW: replaced Latitude_centroid with Latitude_centroid
...
## SR total
library(reshape)
coefs = coefficients(final_model_glmmTMB_Richness_mammal)[[1]][[1]][,-1] ## NEW added another [[1]]
...
## SR bats
library(reshape)
coefs = coefficients(final_model_glmmTMB_Richness_bat)[[1]][[1]][,-1]
colnames(coefs) = c("Area", "SLMP", "Temperature mean", "Temperature sd", "Past_isolation",
                    "CCVT", "Precipitation sd", "Precipitation mean", "Elevation sd", "Area*SLMP")
coefs = cbind(coefs, rownames(coefs))
colnames(coefs)[ncol(coefs)] = "realm"
...
## pSIE all
library(reshape)
coefs = coefficients(final_model_pSIE_mammal)[[1]][,-1]
colnames(coefs) = c("Area", "SLMP", "Temperature mean", "Temperature sd", "Past_isolation",
...
# SIE all
classes = classIntervals(island_data_endemism$SIE_mammal, n = 7, style = "jenks")
breaks = range01(classes$brks)
pdf("SIE_mammal_plot_size_variation.pdf", onefile = T, width = 15, height = 7, useDingbats=FALSE)
ggplot(PNAS_island_subset@data, aes(Longitude, Latitude)) + theme_void() + 
  geom_polygon(data = continents, aes(x = long, y = lat, group = group), fill = "gray90", colour = "gray20", size = 0.005) +
  geom_polygon(data = PNAS_island_subset, aes(x = long, y = lat, group = group), fill = "gray90", colour = "gray20", size = 0.005) +
  geom_exec(geom_point, data = island_data_endemism, size = "SIE_mammal", fill = "SIE_mammal", alpha = 0.8, shape = 21, color = "black") +
  scale_size(range = c(3, 13), breaks = classes$brks) +
  scale_fill_viridis_c(breaks = classes$brks, 
                       values = rev(breaks), guide = "legend")
dev.off()
